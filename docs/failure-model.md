...
| Agent or tool failure inside a task | Retry until `max_attempts` is spent, then dead-letter. | **Implemented.** A failure another attempt might survive records `RETRYING` and leaves the message unacknowledged, so the same lease recovery that covers a crash redelivers it; a failure no retry can change (unregistered agent, capability denial) records `FAILED` immediately. Neither holds its investigation open once resolved. **Gap:** reaching `DEAD_LETTER` now increments `legion.investigation.tasks{outcome="dead_letter"}`, but nothing collects that counter and nothing alerts on it, and nothing reads the row. |
