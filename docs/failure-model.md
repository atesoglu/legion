...
| Agent or tool failure inside a task | Retry until `max_attempts` is spent, then dead-letter. | **Implemented.** A failure another attempt might survive records `RETRYING` and leaves the message unacknowledged, so the same lease recovery that covers a crash redelivers it; a failure no retry can change (unregistered agent, capability denial) records `FAILED` immediately. Neither holds its investigation open once resolved. **Gap:** reaching `DEAD_LETTER` now increments `legion.investigation.tasks{outcome="dead_letter"}`, and the local `deploy/observability/` stack proves that counter is collectible, but no running deployment collects it, nothing alerts on it, and nothing reads the row. |
| Task queue redelivery of a completed task | The task handler is idempotent; a duplicate delivery must not duplicate evidence, findings, or a case. | **Implemented**: `investigation/worker` checks `store.IsTerminal` before doing any work and runs no agent for a task already finished. It does re-run the investigation-completion check, which is idempotent (`CompleteInvestigationIfDone` reports the transition, not the state, so no duplicate audit event is written). |
...
| Database unavailable during case/evidence/finding write | Do not acknowledge the queue message; the task remains claimable once the database recovers. | **Implemented.** `handleTask` returns a disposition: `settled` (acknowledge) only when the outcome is durable in Postgres or never could be — an unparseable envelope, an unknown task, a terminal task, or a recorded success/failure. Every other path returns `retry` and the message is left in the group's pending list for `XAUTOCLAIM` to redeliver once the lease expires. Unit-tested in `investigation/worker/main_test.go` with a faked store, which is the only way to inject a mid-task outage — the e2e suite cannot. Two consequences are accepted rather than solved: redelivery after a partially-completed task re-runs the agent (at-least-once is the contract; `max_attempts` bounds it), and while the database is down the queue drains into the pending list, only reclaimed once `Read` returns empty. |
